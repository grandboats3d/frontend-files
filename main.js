document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchDataSequentially() {
        ...
        /*----------  Initial options  ----------*/
        const boatInitialColorsAndOptions =
            boatData["initial-colors-and-options"];
        ...
        function renderColorControls(
            ...
            colorsArray.forEach((color) => {
                ...
                if (
                    Array.isArray(deactivateColors) &&
                    deactivateColors.length > 0
                ) {
                ...
                name.textContent =
                    color["color-name"] || color["color-id"] || "";
    ...
    function colorFilter() {
        ...
        deactivateColorsButtons.forEach((deactivateBtn) => {
            const siblings = Array.from(
                deactivateBtn.parentElement.children,
            ).filter(
                (sibling) =>
                    sibling !== deactivateBtn &&
                    !sibling.dataset.deactivateColors,
            );
            ...
            const buttonsToDeactivateIds =
                deactivateBtn.dataset.deactivateColors
                    .split(",")
                    .map((x) => x.trim());

            const currentButtonsToDeactivate = colorButtons.filter(
                (toDeactivateBtn) =>
                    buttonsToDeactivateIds.some((id) =>
                        toDeactivateBtn.id.includes(id),
                    ),
            );
            ...
            currentButtonsToDeactivate.forEach((btn) => {
                ...
                if (group) {
                    ...
                    siblings.forEach((sibling) =>
                        buttonsToDeactivate.add(sibling),
                    );
                }
            });
        ...
        relatedToColorOptionParents.forEach((parent) => {
            parent.addEventListener("click", () => {
                ...
                const firstRelatedColorId = relatedColorsIds
                    .split(",")[0]
                    .trim();
                ...
                const matchingRelatedColorOptions = Array.from(
                    document.querySelectorAll(
                        "[data-option-btn][data-related-options]",
                    ),
                ...
                if (parentId.startsWith("d_")) {
                    matchingRelatedColorOptions.forEach((colorOption) => {
                        if (colorOption.classList.contains("is-active")) {
                            ...
                            colorOption.id = colorOption.id =
                                "d_" + currentId.slice(2);
                    ...
                    const allIdsStartWithD = matchingRelatedColorOptions.every(
                        (el) => el.id.startsWith("d_"),
                    );
    ...
    function handleNavBtnClicks() {
        if (navComponent) {
            const allBtns = [
                ...navComponent.querySelectorAll("[data-nav-btn]"),
            ];
            const optionItems = [
                ...document.querySelectorAll("[data-options-item]"),
            ];
    ...
    function handlePopups() {
        ...
        document
            ...
            .forEach((el) => {
                el.addEventListener("click", () => {
                    const popupName =
                        el.dataset.popupClose || el.dataset.popupOverlay;
                    const popup = document.querySelector(
                        `[data-popup="${popupName}"]`,
                    );
                    const content = popup.querySelector("[data-popup-content]");
    ...
    function handleOptions() {
        ...
        function updateActiveItem(direction) {
            ...
            const buttonToHide =
                direction === "next" ? nextOptionBtn : prevOptionBtn;
            const buttonToShow =
                direction === "next" ? prevOptionBtn : nextOptionBtn;
        ...
        prevOptionBtn?.addEventListener("click", () =>
            updateActiveItem("prev"),
        );
        nextOptionBtn?.addEventListener("click", () =>
            ...
            controlGroup.addEventListener("click", (event) => {
                ...
                if (isMultiple) {
                    ...
                    if (currentActiveButton) {
                        ...
                        if (currentId.startsWith("e_")) {
                            currentActiveButton.id = currentId.replace(
                                /^e_/,
                                "d_",
                            );
                        }
                    ...
                    summaryForm.querySelector(`[name="${fieldName}"]`).value =
                        fieldValue;
    ...
    checkFeaturesElements.forEach((el) => {
        el.addEventListener("click", (e) => {
            ...
            const toggleFeaturesBtn =
                document.getElementById("e_toggle_features");
